...
DECOYS = [
    only_letters(
        "THEQUICKBROWNFOXJUMPSOVERTHELAZYDOGAGAINANDAGAINUNTILTHEMORNINGLIGHT"
        "FELLACROSSTHEQUIETFIELDSWHERETHEHARVESTHADLONGSINCEBEENGATHEREDIN"
    ),
    only_letters(
        "LOREMIPSUMDOLORSITAMETCONSECTETURADIPISCINGELITSEDDOEIUSMODTEMPORINCI"
...
def _synth(plaintext, keyword="ZEBRAS", convention="vigenere", alphabet="KRYPTOS"):
    ...
    ct = encode_substitution(
        encode_columnar(pt, keyword), KEY, substitution=convention, alphabet=alphabet
    )
    return pt, ct
...
def test_screen_ranks_true_keytext_and_peels(plaintext):
    ...
    res = screen_running_keys(ct, [KEY, *DECOYS], labels=["true", "d0", "d1"], scorer=get_scorer())
...
def test_key_index_survives_no_letter_drop(plaintext):
    ...
    _, ct = _synth(plaintext)
    res = screen_running_keys(
        ct,
        ["12345", DECOYS[0], KEY],  # entry 0 has no letters -> dropped
        labels=["empty", "decoy", "true"],
        scorer=get_scorer(),
    )
    assert res["winner"]["label"] == "true"
